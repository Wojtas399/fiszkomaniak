...
void main() {
  ...
  test(
    'should return stream which contains appearance settings',
    () async {
      final AppearanceSettings expectedSettings = createAppearanceSettings();
      when(
        () => settingsInterface.appearanceSettings$,
      ).thenAnswer((_) => Stream.value(expectedSettings));

      final Stream<AppearanceSettings> settings$ = useCase.execute();
